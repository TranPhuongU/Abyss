...
[NodeDescription(name: "AirTeleportAction", story: "Air Teleport Action", category: "Action", id: "e61a5a52e9867c43efbf98365a0ab42e")]
public partial class AirTeleportAction : Action
{
    ...
    [SerializeReference] public BlackboardVariable<float> MaxMovingPlayerOffset;
    [SerializeReference] public BlackboardVariable<float> StillPlayerOffset;
    [SerializeReference] public BlackboardVariable<float> MaxStillPlayerOffset;

    // NEW: Y offset for teleport position
    [SerializeReference] public BlackboardVariable<float> TeleportYOffset;

    // NEW: Use random X position ranges
    [SerializeReference] public BlackboardVariable<bool> UseRandomXRange;
    ...
    private float CalculateXOffset()
    {
        ...
        float maxMovingOffset = MaxMovingPlayerOffset?.Value ?? 3.5f;
        float stillOffset = StillPlayerOffset?.Value ?? 1.6f;
        float maxStillOffset = MaxStillPlayerOffset?.Value ?? 2.6f;

        // Check if using random X range mode
        bool useRandomRange = UseRandomXRange?.Value ?? false;
        ...
        if (Mathf.Abs(playerInputDirection) > 0.1f)
        {
            ...
            if (useRandomRange)
            {
                // Random between MovingPlayerOffset and MaxMovingPlayerOffset
                offsetToUse = UnityEngine.Random.Range(movingOffset, maxMovingOffset);
                // Random left/right positioning (positive or negative)
                return UnityEngine.Random.Range(0, 100) < 50 ? offsetToUse : -offsetToUse;
            }
            else
            {
                // Use original MovingPlayerOffset with player input direction
                offsetToUse = movingOffset;

                // Position ahead of player's movement direction
                if (!bossScript.SomethingIsAround())
                {
                    // Position in player's input direction (intercept path)
                    return playerInputDirection * offsetToUse;
                }
                else
                {
        ...
        else
        {
            ...
            if (useRandomRange)
            {
                // Random between StillPlayerOffset and MaxStillPlayerOffset
                offsetToUse = UnityEngine.Random.Range(stillOffset, maxStillOffset);
            }
            else
            {
                // Use original StillPlayerOffset
                offsetToUse = stillOffset;
            }

            // Random left/right positioning
            return UnityEngine.Random.Range(0, 100) < 50 ? offsetToUse : -offsetToUse;
        }
    }
